...
import com.olive.u_share.model.ShareModel;
...
/**
 * Created by tangyunfei on 2017/11/21.
 */
public class UShare {
            ...
            for (int i = 0, j = permissions.length; i < j; i++) {
                if(TextUtils.equals(permissions[i],Manifest.permission.WRITE_EXTERNAL_STORAGE)){
                    if (grantResults[i] == PackageManager.PERMISSION_GRANTED) {
                        ...
                        if(mActivity==null)return;
                        Toast.makeText(mActivity.get(),"没有使用SD卡的权限，请在权限管理中开启使用SD卡的权限",Toast.LENGTH_SHORT).show();
                    }
                }
            }
        }
