...
import {LazyloadView,LazyloadImage} from '../common/lazyload';//LazyloadImage host={lazyloadName}
...
export default class HuodongItem extends Component {
    ...
    render(){
        ...
        return (
            <Card containerStyle={{marginLeft:8,marginRight:8,marginTop:10}} image={require('../../assets/images/nopic4.png')}>
                <TouchableWithoutFeedback onPress={()=>{this.props.navigation.navigate("HuodongDetail",{id:item.id})}}>
                    <View style={{height:150,width:width-18,flex:1,backgroundColor:"transparent",position:'absolute',top:-150,left:0}}>
                        <CachedImage style={{height:150,width:width-18}} source={item['pic'] ? {uri:getFullPath(item['pic'],this.state.host)} : require('../../assets/images/nopic1.jpg')} />
                    </View>
                </TouchableWithoutFeedback>
                <TouchableWithoutFeedback onPress={()=>{this.props.navigation.navigate("HuodongDetail",{id:item.id})}}>
                    <View>
                        ...
                        <View style={styles.huodongItemTime}>
                            <CachedImage style={styles.huodongItemTimeImage} source={require('../../assets/icon/icontime.png')}/>
                        ...
                        <View style={styles.baoming}>
                            <View style={styles.author}>
                                <CachedImage style={{width:30,height:30,marginRight:5}} source={require('../../assets/icon/iconguan.png')}/>
                                <Text style={[colors.cBlue,styles.name]}>{item['name'] ? item['name'] : '官方'}</Text>
                                <Text style={colors.c7b}>发布了该活动</Text>
                                <CachedImage style={[styles.huodongItemTimeImage,{marginLeft:4}]} source={require('../../assets/icon/iconaddress.png')}/>
