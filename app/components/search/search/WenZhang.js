...
import {getNewsList} from '../searchAction';
...
class WenZhang extends Component{
    ...
    componentDidMount(){
        ...
        getNewsList(keyword,page,(content)=>{this.loadUserComplete(content)});
    }
    ...
    _contentViewScroll = (e)=>{
        ...
        if (offsetY + oriageScrollHeight >= contentSizeHeight){
            if(this.state.isFinished === false){
                getNewsList(this.state.keyword,this.state.currentPage,(content)=>{this.loadUserComplete(content)});
            }
        }
    };
    renderRow = ({item}) => (
        <TouchableWithoutFeedback key={item.id} onPress={()=>{this.props.screenProps.navigation.navigate("NewsDetail",{id:item.id})}}>
