...
import {getDongtaiList} from '../searchAction';
...
class DongTai extends Component{
    ...
    componentDidMount(){
        ...
        getDongtaiList(keyword,page,(content)=>{this.loadUserComplete(content)});
    }
    ...
    _contentViewScroll = (e)=>{
        ...
        if (offsetY + oriageScrollHeight >= contentSizeHeight){
            if(this.state.isFinished === false){
                getDongtaiList(this.state.keyword,this.state.currentPage,(content)=>{this.loadUserComplete(content)});
            }
        }
    };
    renderRow = ({item}) => (
        <TouchableWithoutFeedback key={item.id} onPress={()=>{this.props.screenProps.navigation.navigate("DongTaiDetail",{id:item.id})}}>
            <View style={{flexDirection:'row',alignItems:'center',height:100,borderBottomWidth:1,borderBottomColor:'#f2f2f2'}}>
                ...
                <View>
                    ...
                    <Text style={{color:'#555',width:width-100}}>{strCut(item.content,80)}</Text>
                </View>
            </View>
        </TouchableWithoutFeedback>
    );
